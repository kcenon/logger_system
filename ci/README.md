...
```sh
python3 scripts/check_tag_reality.py --repo kcenon/logger_system --tag v1.2.3 \
  --release-commit FULL_RELEASE_MERGE_SHA \
  --port-dir vcpkg-ports/kcenon-logger-system
```
...
- `LOGGER_USE_THREAD_SYSTEM` → `KCENON_WITH_THREAD_SYSTEM`.
