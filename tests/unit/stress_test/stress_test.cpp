...
protected:
    void SetUp() override {
        ...
        test_dir_ = "/tmp/logger_stress_test_" +
...
TEST_F(stress_test, buffer_overflow_stress) {
    ...
    // Wait for writer to catch up by polling
    auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
    while (slow_writer_ptr->get_write_count() < 10 && std::chrono::steady_clock::now() < deadline) {
    ...
    std::cout << "Buffer overflow test: Sent " << messages_sent.load() << ", Written "
              << written_count << ", Dropped " << (messages_sent.load() - written_count)
              << std::endl;
}
...
TEST_F(stress_test, writer_switching_stress) {
    auto result = logger_module::logger_builder().with_default_pattern().build();
...
TEST_F(stress_test, random_load_pattern_stress) {
    ...
    // Allow time for final messages to be written (poll completion)
    auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
    while (mock_writer_ptr->get_write_count() < message_count &&
    ...
    std::cout << "Random load test: Processed " << message_count << " messages in "
...
TEST_F(stress_test, async_writer_performance_stress) {
    ...
    auto async_writer_inst =
        std::make_unique<logger_module::async_writer>(std::move(file_writer_inst), 1000);

    // Need to keep a raw pointer for later use
    auto* async_writer_ptr = async_writer_inst.get();

    auto result =
        logger_module::logger_builder().add_writer("async", std::move(async_writer_inst)).build();
