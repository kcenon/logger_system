...
TEST_F(CrashSafetyTest, Initialization) {
    ...
    crash_safety.initialize(logger_crash_safety_level::standard, test_log_path_, 1000);
...
TEST_F(CrashSafetyTest, LoggerRegistration) {
    ...
    crash_safety.register_logger(
        "test_logger", [&flush_called]() { flush_called = true; },
...
TEST_F(CrashSafetyTest, EmergencyLogging) {
    ...
    while (!std::filesystem::exists(test_log_path_) &&
...
TEST_F(CrashSafetyTest, AutoBackup) {
    ...
    int backup_count = 0;
    crash_safety.register_logger(
        "auto_backup_test", []() {}, [&backup_count](const std::string&) { backup_count++; });
...
TEST_F(CrashSafetyTest, Statistics) {
    ...
    crash_safety.register_logger("stats_test", []() {}, [](const std::string&) {});
...
TEST_F(CrashSafetyTest, ScopedProtection) {
    ...
    {
        scoped_logger_crash_protection protection("scoped_test",
...
TEST_F(AsyncLoggerCrashSafetyTest, OverflowHandler) {
    ...
    async_logger_crash_safety::set_overflow_handler(
        "async_logger", [&overflow_called, &dropped_count](size_t dropped) {
