...
TEST_F(MonitoringIntegrationTest, PerformanceMetricsCollection) {
    ...
    monitoring_system_->record_metric("logger.throughput", throughput, {{"unit", "msg/s"}});
    ...
    auto throughput_metric = std::find_if(metrics.begin(), metrics.end(), [](const auto& m) {
        return m.name == "logger.throughput";
    });
...
TEST_F(MonitoringIntegrationTest, AlertSystemIntegration) {
    ...
    if (error_count > 5) {
        monitoring_system_->send_alert(
            "logger.high_error_rate",
            "Error count exceeded threshold: " + std::to_string(error_count), "critical");
    }

    // Simulate health check failure
    monitoring_system_->send_alert("logger.health_check_failed",
                                   "Logger health check failed: high error rate", "warning");
...
TEST_F(MonitoringIntegrationTest, MetricsAggregation) {
    ...
    monitoring_system_->record_metric("logger.total_logs_aggregated",
                                      static_cast<double>(total_logs), {{"aggregation", "sum"}});
...
TEST_F(MonitoringIntegrationTest, RealTimeMonitoring) {
    ...
    std::thread monitoring_thread([this, &callback_count, &last_total_logs, &monitoring_active]() {
        ...
        while (monitoring_active) {
            ...
            if (current_logs > last_total_logs) {
                monitoring_system_->record_metric(
                    "logger.realtime.rate", static_cast<double>(current_logs - last_total_logs),
                    {{"interval", "100ms"}});
    ...
    auto metrics = monitoring_system_->get_metrics();
    auto realtime_metrics = std::count_if(metrics.begin(), metrics.end(), [](const auto& m) {
        return m.name.find("realtime") != std::string::npos;
    });
...
TEST_F(MonitoringIntegrationTest, CustomMetricsIntegration) {
    ...
    monitoring_system_->record_metric("logger.custom.counter", static_cast<double>(custom_counter),
                                      {{"type", "counter"}});
    monitoring_system_->record_metric("logger.custom.gauge", custom_gauge, {{"type", "gauge"}});

    double avg = static_cast<double>(custom_histogram_sum) / custom_histogram_count;
    monitoring_system_->record_metric("logger.custom.histogram_avg", avg, {{"type", "histogram"}});

    // Verify custom metrics
    auto metrics = monitoring_system_->get_metrics();
    auto custom_metric_count = std::count_if(metrics.begin(), metrics.end(), [](const auto& m) {
